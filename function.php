...
if(isset($_POST['btn_add'])){
    ...
    if($num_rows == 0){
        ...
        $query = mysqli_query($con,"INSERT INTO tblclearance (clearanceNo,residentid,purpose,samount,dateRecorded,recordedBy,status) 
            values ('$txt_cnum','$ddl_resident','$txt_purpose',  '$txt_amount', '$date', '".$_SESSION['username']."','Approved')") or die('Error: ' . mysqli_error($con));
        }
        else{
        $query = mysqli_query($con,"INSERT INTO tblclearance (clearanceNo,residentid,purpose,samount,dateRecorded,recordedBy,status) 
            values ('$txt_cnum','$ddl_resident', '$txt_purpose',  '$txt_amount', '$date', '".$_SESSION['username']."','New')") or die('Error: ' . mysqli_error($con));
        }
        if($query == true)
        {
            $_SESSION['added'] = 1;
    ...
    if($num_row > 0)
    {
        ...
        while($row = mysqli_fetch_array($chk)){
            ...
            if($row['lengthofstay'] < 6){
                ...
                $txt_purpose = $_POST['txt_purpose'];
                $date = date('Y-m-d');
                $reqquery = mysqli_query($con,"INSERT INTO tblclearance (clearanceNo,residentid,purpose,samount,dateRecorded,recordedBy,status) 
...
if(isset($_POST['btn_approve']))
{
    ...
    $approve_query = mysqli_query($con,"UPDATE tblclearance set clearanceNo= '".$txt_cnum."',  samount = '".$txt_amount."', status='Approved' where id = '".$txt_id."' ") or die('Error: ' . mysqli_error($con));
...
if(isset($_POST['btn_save']))
{
    ...
    $update_query = mysqli_query($con,"UPDATE tblclearance set clearanceNo= '".$txt_edit_cnum."', purpose = '".$txt_edit_purpose."',  samount = '".$txt_edit_amount."' where id = '".$txt_id."' ") or die('Error: ' . mysqli_error($con));
