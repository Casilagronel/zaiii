                ...
                <div class="row">
                    <div class="box">
                       <div class="box-header">
                            ...
                            <button class="btn btn-success btn-sm" data-toggle="modal" data-target="#addModal"><i class="" aria-hidden="true"></i> Add Clearance</button>  
                            <?php 
                                ...
                                $squery = mysqli_query($con, "SELECT *,CONCAT(r.lname, ', ' ,r.fname, ' ' ,r.mname) as residentname,p.id as pid FROM tblclearance p left join tblresident r on r.id = p.residentid  where status = 'Approved'") or die('Error: ' . mysqli_error($con));
                                                    while($row = mysqli_fetch_array($squery))
                                                    {
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td><button class="btn btn-success btn-sm" data-target="#editModal'.$row['pid'].'" data-toggle="modal"><i class="" aria-hidden="true"></i> Edit</button> | 
                                                            <a target="" href="clearance_form.php?resident='.$row['residentid'].'&clearance='.$row['clearanceNo'].'&val='.base64_encode($row['clearanceNo'].'|'.$row['residentname'].'|'.$row['dateRecorded']).'" onclick="location.reload();" class="btn btn-warning btn-sm"><i class="" aria-hidden="true"></i> Generate</a></td>
                                                        </tr>
                                                        ';
                                                    ...
                                                    while($row = mysqli_fetch_array($squery))
                                                    {
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td>₱ '.number_format($row['samount'],2).'</td>
                                                            <td><button class="btn btn-success btn-sm" data-target="#editModal'.$row['pid'].'" data-toggle="modal"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Edit</button>
                                                            <a target="_blank" href="clearance_form.php?resident='.$row['residentid'].'&clearance='.$row['clearanceNo'].'&val='.sha1($row['clearanceNo'].'|'.$row['residentname'].'|'.$row['dateRecorded']).'" onclick="location.reload();" class="btn btn-warning btn-sm"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Generate</a></td>
                                                        </tr>
                                                        ';
                                        ...
                                        <table id="table1" class="table table-bordered table-striped"> -->
                                            ...
                                            <tbody>
                                                ...
                                                if(!isset($_SESSION['staff']))
                                                {
                                                    ...
                                                    $squery = mysqli_query($con, "SELECT *,CONCAT(r.lname, ', ' ,r.fname, ' ' ,r.mname) as residentname,p.id as pid FROM tblclearance p left join tblresident r on r.id = p.residentid where status = '' ") or die('Error: ' . mysqli_error($con));
                                                    while($row = mysqli_fetch_array($squery))
                                                    {
                                                        echo '
                                                        <tr>
